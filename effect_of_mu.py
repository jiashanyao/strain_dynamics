...
if __name__ == '__main__':
    ...
    for dur in dur_samples:
        pool.apply_async(simulate, args=(CONTACTS_PER_HOST, 1/dur, SIGMA, BETA, R, TAU, GAMMA, N_LOCI, N_NODES, SEEDS_PER_STRAIN, RANDOMNESS, N_STEPS), callback=lambda result: log_result(result, result_list))
    pool.close()
    pool.join()

    # plot
    result_list.sort(key=lambda d: d['mu'])
    plt.figure(figsize=(4, 2))
    diversity = [d['mean_diversity'] for d in result_list]
    discordance = [d['mean_discordance'] for d in result_list]
    dur_list = [1 / d['mu'] for d in result_list]
